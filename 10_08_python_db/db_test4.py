...
def score_input(stu_id):
    # 입력한 학번이 DB에 있는지 확인하기    
    chk_student = []
    cur.execute("select * from students")
    ...
    # 성적 테이블에 성적이 입력되었는지 확인하기
    chk_score = []
    cur.execute("select * from score")
    for row in cur.fetchall():
        chk_score.append(row[0])
    
    chk_student # 리스트에 학번이 있으면 참, 없으면 거짓
    if stu_id in chk_student:
        print("학번이 존재합니다... ")

        # chk_score리스트에 학번이 있으면 참, 없으면 거짓
        if stu_id in chk_score:
            print('이미 성적이 입력되었습니다!!!')
        else:
            print('성적을 입력하세요!!')
            os = int(input('운영체제 : '))
            ...
            cur.execute("insert into score values(?,?,?,?)", (stu_id, os, comVision, db))
            print("성적 입력 완료!!")
    else:
