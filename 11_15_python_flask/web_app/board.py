...
def comment_list(root_id):
    ...
    for comment in comments:
        ...
        # else:
        #     owner = False
        owner = True if comment.get("writer_id") == session.get("id") else False
...
def comment_delete():
    if request.method == "POST":
        ...
        doc = comment.find_one({"_id": ObjectId(idx)})
        if doc.get("writer_id") == session.get("id"):
...
##################### 댓글 수정 ###################
@bp.route("/comment_modify", methods=["POST"])
def comment_modify():
    if request.method == "POST":
        ...
        doc = collection_comment.find_one({"_id": ObjectId(idx)})
        if doc.get("writer_id") == session.get("id"):
